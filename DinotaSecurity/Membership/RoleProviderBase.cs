...
namespace Dinota.Security.Membership
{
    public abstract class RoleProviderBase : RoleProvider
    {
        ...
        {
            using (var securityContext = GetSecurityContext())
            {
                var roleService = GetRoleService(securityContext);

                return roleService.GetUserRoles(username).Where(role => role.Name == roleName).Count() > 0;
            }
        }
        ...
        public override string[] GetRolesForUser(string username)
        {
            using (var securityContext = GetSecurityContext())
            {
                ...
                return roleService.GetUserRoles(username).Select(role => role.Name).ToArray();
            }
        }
        ...
        protected abstract RoleService GetRoleService(ISecurityContext securityContext);
    }
}
