            ...
            if (ModelState.IsValid)
            {
                if (Membership.ValidateUser(model.UserName, model.Password))
                {
                    ...
                    FormsAuthentication.SetAuthCookie(model.UserName, model.RememberMe);
                        ...
                        {
                            return RedirectToAction("SLAFLocationSummary", "Report", new { archive = 0 });
                        }
                        else if (user.UserTypeId == 4)
                        {
                            return RedirectToAction("SelfRegistration", "Vehicle", new { archive = 0 });
                        }
                        else
                        {
                            return RedirectToAction("Index", "Home", new { archive = 0 });
                        }
